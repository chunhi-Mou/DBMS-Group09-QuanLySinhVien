...
BEGIN
    ...
    DECLARE v_dangky_id INT DEFAULT NULL;
    ...
    SELECT mkh.monhoc_id, mkh.kihoc_id
      INTO v_monhoc_id, v_kihoc_id
      FROM LopHocPhan lhp
      JOIN MonHoc_KiHoc mkh ON mkh.id = lhp.monhockihoc_id
     WHERE lhp.id = p_lophocphan_id;

    SELECT id INTO v_dangky_id
      FROM DangKyHoc
     WHERE sinhvien_id = p_sinhvien_id
       AND lophocphan_id = p_lophocphan_id;

    IF v_dangky_id IS NOT NULL THEN
        IF EXISTS (
    ...
    IF EXISTS (
        SELECT 1
         ...
         WHERE dkh.sinhvien_id = p_sinhvien_id
           AND dkh.trangthai <> 'Đã hủy'
           AND dkh.lophocphan_id <> p_lophocphan_id
           AND mkh.monhoc_id = v_monhoc_id
           AND mkh.kihoc_id = v_kihoc_id
    ) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Đã đăng ký';
    END IF;
         ...
         WHERE bh_moi.lophocphan_id = p_lophocphan_id
           AND EXISTS (
               SELECT 1
                 ...
                 JOIN LopHocPhan lhp ON lhp.id = dkh.lophocphan_id
                 JOIN MonHoc_KiHoc mkh ON mkh.id = lhp.monhockihoc_id
                 JOIN BuoiHoc bh_cu ON bh_cu.lophocphan_id = lhp.id
                WHERE dkh.sinhvien_id = p_sinhvien_id
                  AND dkh.trangthai <> 'Đã hủy'
                  AND dkh.lophocphan_id <> p_lophocphan_id
                  AND mkh.kihoc_id = v_kihoc_id
